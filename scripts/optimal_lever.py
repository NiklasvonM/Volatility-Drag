...
scenarios = [
    ...
    Scenario(volatility=0.30, expected_return=0.08 - 0.0325),
...
def main() -> None:
    for scenario in scenarios:
        theoretically_best_lever = round(
            np.log(1 + scenario.expected_return) / scenario.volatility**2, 2
        )
        levers[-1] = theoretically_best_lever
    sorted_levers = sorted(levers)
    for scenario in tqdm(scenarios):
        ...
        max_median = medians[theoretically_best_lever]
        plt.plot(theoretically_best_lever, max_median, "o", color="black")
    plt.title(
